...
def process_tokens(all_tokens, lang):
    ...
    # added a if scenario for CHinese, because spacy and wordfreq used diffent code for CHinese
    if lang == 'ch':
        for tokens in all_tokens:
            doc = Doc(nlp.vocab, words=tokens)
            # ADDED THE FOLLOWING two LINES TO ADDRESS THE TypeError of spacy function calling:
            text = ' '.join([token.text for token in doc])
            processed = nlp(text)
            sentence_tags = [token.pos_ for token in processed]
            sentence_frequencies = [word_frequency(token.lemma_, 'zh', wordlist='best', minimum=0.0) for token in
                                    processed]
        ...
        for tokens in all_tokens:
            ...
            processed = nlp(text)
            sentence_tags = [token.pos_ for token in processed]
            sentence_frequencies = [word_frequency(token.lemma_, lang, wordlist='best', minimum=0.0) for token in
                                    processed]
            pos_tags.append(sentence_tags)
            frequencies.append(sentence_frequencies)
...
def plot_correlation(df, ax, model : str, column: str, title: str):
    ...
    print("x", x)
    print("labels", labels)
    print("Attn last", attn_last)
    print("attn_1st", attn_1st)
    ...
    if 'length' in column.lower() or 'freq' in column.lower():
        ...
        print('x-width', len(x - width))
        print('attn_1st', len(attn_1st))
        print(width)
        # print('width', len(width))
        rects1 = ax.bar(x - width, attn_1st, width, label='Attn (1st)')
        rects2 = ax.bar(x, attn_last, width, label='Attn (last)')
        rects3 = ax.bar(x + width, saliency, width, label='Saliency')
...
def plot_baselines_vs_importance(fn, model : str):
    ...
    #deleted the subplot for word length
    fig, (ax1) = plt.subplots(1)
    formatted_bl_df = format_df(bl_df)
    # plot_correlation(formatted_bl_df, ax1, model, 'length_r_mean', 'Word length')
    plot_correlation(formatted_bl_df, ax1, model, 'freq_r_mean', 'Word frequency')
