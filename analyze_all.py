...
def extract_human_importance(dataset):
    ...
    return tokens, human_importance, sentences
...
def populate_dataframes(corpora_modelpaths, types):
    ...
    for corpus, modelpaths in corpora_modelpaths.items():
        print(corpus)
        et_tokens, human_importance, sentences = extract_human_importance(corpus)
        lang = corpora_languages[corpus]

        pos_tags, frequencies = process_tokens(et_tokens, lang)
        print('freqs', len(frequencies))
        lengths = [[len(token) for token in sent] for sent in et_tokens]
        print(lengths)
        # frequencies = frequencies[:len(et_tokens)]
        for idx, zipped in enumerate(zip(et_tokens, human_importance, frequencies, lengths, sentences, corpus)):

            data = {
                'et_token': zipped[0],
                'et_importance': zipped[1],
                'frequency': zipped[2][:len(zipped[0])],
                'length': zipped[3],
                'sentence': zipped[4],
                'corpus': zipped[5]
            }
            ...
            # print(len(zipped[3]))
            # print(len(data))
            # print(len(human_words_df.columns))
        ...
        for importance_type in types:
            ...
            for mp in modelpaths:
                ...
                aligned = align_sentences(et_tokens, human_importance,
                                          ...
                                          importance_type, corpus, modelname)
                # align_sentences() return 4 elements:
                # aligned_et_tokens, aligned_human_salience, aligned_lm_tokens, aligned_lm_salience
                ...
                # added keys and values to matach the arrays:
                for idx, zipped in enumerate(zip(aligned[0], aligned[1], aligned[2], aligned[3], lm_frequencies, lm_lengths, sentences, corpus, importance_type, modelname)):
                    data = {
                        'et_token': zipped[0],
                        'et_importance': zipped[1],
                        'lm_token': zipped[2],
...
def write_results_to_excel(results):
    ...
    fname = "/results/all_results" + timestr + ".xlsx"
...
if __name__ == '__main__':
    ...
    types = ["saliency", "attention", "attention_1st_layer"]
