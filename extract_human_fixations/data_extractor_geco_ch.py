...
def read_geco_file(reading_data_df, sentence_info_df, task):
    # CHANGE 'PP_NR' TO 'PP_ID' to adapt to the excel sheet of the Chinese data
    subjects = pd.unique(reading_data_df['PP_ID'].values)
    sentences = pd.unique(sentence_info_df['SENTENCE_ID'].values)

    flat_word_index = 0
    for subj in subjects:
        print(subj)
        subj_data_orig = reading_data_df.loc[reading_data_df['PP_ID'] == subj]
        ...
        for j, sent in enumerate(sentences):
            ...
            for k, (w, id) in enumerate(zip(tokens, word_ids)):
                trt = subj_data_orig['IA_DWELL_TIME'].loc[subj_data_orig['WORD_ID'] == id].values
        ...
        df_subj.to_csv("/extract_human_fixations/data/"
                       +task+"/relfix/"+subj+"-relfix-feats.csv")
...
def extract_features(task):
    ...
    sent_dict ={}
    relfix_dir = "/extract_human_fixations/data/" + task + "/relfix/"
    for file in sorted(os.listdir(relfix_dir)):
        print("Reading files for subj ", file)
        subj_data = pd.read_csv(relfix_dir + file, delimiter=',')
    ...
    # write txt files of sentences and relfix in the result folder
...
def main():
    ...
    parser.add_argument('-l', '--language', help="Chinese (ch)", default='ch')
    args = parser.parse_args()
    lang = args.language
    if lang == 'ch':
        reading_data_fn = "/extract_human_fixations/data/geco_ch/raw/L1ReadingData.xlsx"
        print("Reading file for GECO_CN: ", reading_data_fn)
        reading_data_df = pd.read_excel(reading_data_fn, na_filter=False,
                                        usecols="A,E,F,I,J,K,BA", engine='openpyxl')
        # it's important to use the columns representing the same values as in the English data.
        sentence_info_fn = "/extract_human_fixations/data/geco_ch/raw/ChineseMaterial.xlsx"
        print("Reading file for GECO_CN: ", sentence_info_fn)
